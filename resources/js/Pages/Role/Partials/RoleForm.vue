  ...
  <Dialog @show="initForm" :visible="visible" @update:visible="$emit('update:visible', event)" :style="{width: '450px'}"
    ...
    <form @submit.prevent="submit" class="grid formgrid p-fluid">
      ...
      <div class="field mb-4 col-12" v-has-permission="{props: $page.props, permissions: ['permissions.assign']}">
        <FormField :displayErrors="displayErrors" :options="permissions" optionValue="id" optionLabel="type" optionGroupLabel="name" :showToggleAll="false" optionGroupChildren="children"
                   component="MultiSelect" label="Permissions" v-model="localRole.permissions" name="roles"/>
      </div>
    </form>
    <template #footer>
